  ...
  useEffect(() => {
    apiStatus()
      .then((s) => {
        ...
        setSupabaseEnabled(s.supabase);
      })
      .catch(() => undefined);
  }, []);

  async function onPrepared(p: PrepareResponse, _g: Guidelines, m: Mode) {
    setPaywallReason(null);
    setError(null);

    // Gate: only call session/start when Supabase is configured.
    if (supabaseEnabled) {
      const sessionMode = m === "voice" ? "voice" : "text";
      const gate = await apiSessionStart(sessionMode).catch(() => null);
      if (!gate) {
        setError("Could not verify session access — please try again.");
        return;
      }
      if (!gate.allowed) {
        setPaywallReason(gate.reason ?? "no_subscription");
        return;
      }
    }
  ...
  return (
    <AuthGate>
      <main className="mx-auto w-full max-w-3xl flex-1 px-6 py-10 sm:py-14">
        ...
        {paywallReason && <Paywall reason={paywallReason} />}

        {!paywallReason && step === "config" && (
        ...
        {!paywallReason && step === "interview" && prep && plan && (
